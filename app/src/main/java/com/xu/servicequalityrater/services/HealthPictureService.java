...
/**
 * Custom Picture Service based on the
 */
...
/**
 * Mental health idea.  Need to put in code to actually send to microsoft
 * Note that this is candidate 1, and not an actual service, Another incoming example using Camera 1 at
...
public class HealthPictureService {
    ...
    public void startCapturing(final Activity activity,
        ...
        //TODO this is the part where we get the camera IDs, we need to get front facing camera only
        try {
    ...
    private final CameraDevice.StateCallback stateCallback = new CameraDevice.StateCallback() {
        ...
        public void onOpened(@NonNull CameraDevice camera) {
            ...
            new Handler().postDelayed(new Runnable() {
                public void run() {
                    HealthPictureService.this.takePicture();
                }
            }, 1000);
        ...
        public void onClosed(@NonNull CameraDevice camera) {
            ...
            //Start a new thread if we still have another camera
            if (!cameraIds.isEmpty()) {
                new Handler().postDelayed(new Runnable() {
                    public void run() {
                        HealthPictureService.this.takeAnotherPicture();
                    }
                }, 100);
        ...
        try {
            ...
            reader.setOnImageAvailableListener(new ImageReader.OnImageAvailableListener() {
                public void onImageAvailable(ImageReader readerL) {
                    ...
                    buffer.get(bytes);
                    HealthPictureService.this.saveImageToDisk(bytes);
                    if (image != null) {
                        image.close();
                    }
                }
